...
def independent_pair(a1, a2):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Inconsistent effect check
    positive_effects_a1 = a1.get_add()
    positive_effects_a2 = a2.get_add()

    for prop in positive_effects_a1:
        if a2.is_neg_effect(prop):
            return False
    for prop in positive_effects_a2:
        if a1.is_neg_effect(prop):
            return False
    # Interfere check
    a1_pre_condtions = a1.get_pre()
    a2_pre_condtions = a2.get_pre()

    for prop in a1_pre_condtions:
        if a2.is_neg_effect(prop):
            return False
    for prop in a2_pre_condtions:
        if a1.is_neg_effect(prop):
            return False
    return True
